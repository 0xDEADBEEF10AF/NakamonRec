            ...
            VStack(alignment: .leading, spacing: 1) {
                if let s = stats {
                    ...
                    Text("\(s.matches)Matches")
                        .font(.caption2.monospacedDigit())
                        .foregroundStyle(.gray)
                    Text("\(s.wins)W-\(s.losses)L(Use:\(RateFormat.percent(s.usageRate)))")
                        .font(.caption2.monospacedDigit())
                        .foregroundStyle(.gray)
        ...
        case .past(let row):
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text("\(row.stats.matches)Matches")
                    .font(.caption2.monospacedDigit())
                    .foregroundStyle(.gray)
                Text("\(row.stats.wins)W-\(row.stats.losses)L(Use:\(RateFormat.percent(row.stats.usageRate)))")
                    .font(.caption2.monospacedDigit())
                    .foregroundStyle(.gray)
